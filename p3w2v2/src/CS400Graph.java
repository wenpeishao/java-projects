...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence = new LinkedList<>();
            int i = 0;
            while(i<copyPath.dataSequence.size()) {
                if (!this.dataSequence.contains(copyPath.dataSequence.get(i))) {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) throws NoSuchElementException{
        ...
        if (!this.containsVertex(start)) {
            throw new NoSuchElementException("no path from start to end can be found,including when no vertex containing start or end can be found");
        }
        if(!this.containsVertex(end)) {
          throw new NoSuchElementException("no path from start to end can be found,including when no vertex containing start or end can be found");
        }
        PriorityQueue<Path> pq = new PriorityQueue<Path>(); 
        Path initial = new Path(new Vertex(start)); // Initialize the first possible path
        Vertex cur = this.vertices.get(start); //Initialize the current path
        Vertex  finale = this.vertices.get(end); // Initialize the last
        Hashtable<Vertex, Double> dis = new Hashtable<Vertex, Double>();
        Path sp = initial; 
        dis.put(cur, initial.distance); 
        for (int i=0; i<cur.edgesLeaving.size(); i++) {
          pq.add(new Path(initial, cur.edgesLeaving.get(i)));
        }
        while (!pq.isEmpty() && cur != finale) {
            Path currentPath = pq.peek();
            cur = pq.poll().end; 
            if (dis.containsKey(cur)) { //Skip the already existing
                continue; 
            }
            if (cur != finale) { // Determine if it is the end
                for (Edge e: cur.edgesLeaving) {
                  pq.add(new Path(currentPath, e));
                }
            }
            sp = currentPath; 
            dis.put(cur, sp.distance); 
        }
        if (!dis.containsKey(finale)) {
            throw new NoSuchElementException("no path from start to end can be found,including when no vertex containing start or end can be found");
        }
        return sp;
    }
